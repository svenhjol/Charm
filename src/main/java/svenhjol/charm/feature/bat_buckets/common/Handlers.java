...
public final class Handlers extends Handler<BatBuckets> {
    ...
    public void playGrabSound(ServerLevel level, BlockPos pos) {
        level.playSound(null, pos, BatBuckets.registers.grabSound.get(),
            SoundSource.PLAYERS, 0.6f, 0.95f + level.getRandom().nextFloat() * 0.2f);
    }

    public void playReleaseSound(ServerLevel level, BlockPos pos) {
        level.playSound(null, pos, BatBuckets.registers.releaseSound.get(),
            SoundSource.PLAYERS, 0.6f, 0.95f + level.getRandom().nextFloat() * 0.2f);
    }
    ...
    public ItemStackResult useItemInHand(Level level, Player player, InteractionHand hand) {
        ...
        if (!player.level().isClientSide()) {
            BatBuckets.advancements.usedBatBucket(player);
            player.addEffect(new MobEffectInstance(Echolocation.mobEffect.get(), GLOW_TIME * 20));
        }
    ...
    public boolean useBucketOnBat(Player player, ItemStack bucket, InteractionHand hand, Bat bat) {
        var batBucket = new ItemStack(BatBuckets.registers.bucketItem.get());
        ...
        player.getCooldowns().addCooldown(BatBuckets.registers.bucketItem.get(), 30);
        player.swing(hand);
        bat.discard();

        BatBuckets.advancements.capturedBat(player);
        return true;
    }
