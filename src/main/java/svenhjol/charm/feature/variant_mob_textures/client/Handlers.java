...
public final class Handlers extends FeatureHolder<VariantMobTextures> {
    ...
    public Handlers(VariantMobTextures feature) {
        super(feature);
    }
    ...
    private ResourceLocation texture(Entity entity, List<ResourceLocation> normalSet, List<ResourceLocation> rareSet) {
        ...
        var isRare = VariantMobTextures.rareVariantChance > 0
            && !rareSet.isEmpty()
            && (id.getLeastSignificantBits() + id.getMostSignificantBits()) % VariantMobTextures.rareVariantChance == 0;
