...
public class CoralSquidEntityModel<T extends Entity> extends HierarchicalModel<T> {
    ...
    public static LayerDefinition getTexturedModelData() {
        ...
        var cubeListBuilder = CubeListBuilder.create().texOffs(48, 0).addBox(-0.5F, 0.0F, -0.5F, 1.0F, 7.0F, 1.0F);

        for (int k = 0; k < 8; ++k) {
            var d = (double)k * 3.141592653589793D * 2.0D / 8.0D;
            var f = (float)Math.cos(d) * 2.5F;
            var h = (float)Math.sin(d) * 2.5F;
    ...
    public void setupAnim(T entity, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch) {
        var var7 = this.tentacles;
        var var8 = var7.length;
