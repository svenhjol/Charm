...
public class AtlasRenderer implements AutoCloseable {
    ...
    public void renderAtlas(ClientPlayerEntity player, AtlasInventory inventory, MatrixStack matrixStack, VertexConsumerProvider buffers, int light) {
        this.getInstance(inventory).renderAtlas(inventory.getActiveMapId(player), inventory.getActiveMap(player), matrixStack, buffers, light);
    }
    ...
    public void renderArm(ClientPlayerEntity player, MatrixStack matrixStack, VertexConsumerProvider buffers, int light, float swing, float equip, Hand hand) {
        ...
        matrixStack.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(e * 45.0F));
        float e5 = MathHelper.sin(swing * swing * (float) Math.PI);
        float e6 = MathHelper.sin(e1 * (float) Math.PI);
        matrixStack.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(e * e6 * 70.0F));
        matrixStack.multiply(Vec3f.POSITIVE_Z.getDegreesQuaternion(e * e5 * -20.0F));
        ...
        matrixStack.multiply(Vec3f.POSITIVE_Z.getDegreesQuaternion(e * 120.0F));
        matrixStack.multiply(Vec3f.POSITIVE_X.getDegreesQuaternion(200.0F));
        matrixStack.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion(e * -135.0F));
    ...
    public class Instance implements AutoCloseable {
        ...
        public void renderAtlas(int mapId, MapState mapData, MatrixStack matrixStack, VertexConsumerProvider buffers, int light) {
            this.renderBackground(matrixStack, buffers, light);

            if (mapData != null) {
                matrixStack.push();
                MinecraftClient.getInstance().gameRenderer.getMapRenderer().draw(matrixStack, buffers, mapId, mapData, false, light);
                matrixStack.pop();
            }
        }
